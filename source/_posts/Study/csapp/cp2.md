...
IEEE浮点标准用`V=(-1)^s^ * M * 2^E^`的形式来表示一个数  
 - 符号：s 决定这个数是正数还是负数；s直接编码符号s
 - 尾数：M 是一个二进制小数，范围为 `1~2-ε` 或者是 `0~1-ε`；k 位的阶码字段 `exp` 编码阶码 E
 - 阶码：E 的作用是对浮点数加权，权重是2的E次幂；n 位的小数字段 `frac` 编码尾数 M
